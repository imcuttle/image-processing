    ...
    public void actionPerformed(ActionEvent evt)
    {
		...
		else if(evt.getSource() == wlsbchaosItem)
		{
			if(loadflag)
			{
				...
				//写入.raw含水印文件
				reader.writeRAW("./images/ch15/water/w_"+imn+".raw",pixels,iw,ih);	
				
				//存储含水印图像为JPEG
			    File file = new File("./images/ch15/jpg/j_"+imn+".jpg");
		...
		else if(evt.getSource() == dlsbchaosItem)
		{
			if(!imh.equals("j_"))                //未受JPEG攻击
        		pixels = reader.readRAW1D("./images/ch15/water/w_"
        		       + imn + ".raw", iw, ih);		        
			else
			{                                    //受JPEG攻击
				pixels = common.grabber(iImage, iw, ih);
		...
		else if(evt.getSource() == wlsbimageItem)
		{
			if(loadflag)
			{
				...
				pixels = common.grabber(common.openImage(
					     "./images/ch15/Bigwater.jpg", tracker), iw, ih);
		...
		{
			pixels = reader.readRAW1D("./images/ch15/water/w_"+imn+".raw", iw, ih);
	        ImageProducer ip = new MemoryImageSource(iw, ih, pixels, 0, iw);
		    oImage = createImage(ip);		
		...
		else if(evt.getSource() == dmItem)
		{
			if(loadflag)
			{
			    ...
			    //读入水印图像
			    pixels = common.grabber(common.openImage(
			    	     "./images/ch15/water.jpg", tracker), 32, 32);
